...
![Kubernetes clients interact with the Control Plane](/docs/assets/software-engineering/software-architecture/architecture-styles/microservice/16-deployment/kubernetes/main components.png)
...
image::16-deployment/kubernetes/image.png[Kubernetes’ main components are the API, the control plane, and the worker nodes]
...
| kubectl logs +++&lt;pod_id>++++++&lt;/pod_id>+++ | Shows the logs for the given Pod |
| kubectl delete deployment +++&lt;name>++++++&lt;/name>+++ | Deletes the given Deployment |
| kubectl delete pod +++&lt;name>++++++&lt;/name>+++ | Deletes the given Pod |
| kubectl delete svc +++&lt;service>++++++&lt;/service>+++ | Deletes the given Service |
| kubectl port-forward svc +++&lt;service>++++++&lt;host-port>+++:+++&lt;cluster-port>++++++&lt;/cluster-port>++++++&lt;/host-port>++++++&lt;/service>+++ | Forwards traffic from your local machine to within the cluster |
...
image::16-deployment/kubernetes/Kubernetes manifest.png[Kubernetes manifest]
...
image::16-deployment/kubernetes/ingress.png[Ingress NGINX routes traffic to the Edge Service, which in turn routes it to the Catalog Service and the Order Service]
