...
the special <q>bootstrap</q> phase. It uses this phase because you want Spring Boot to apply
...
YAML content.
image::02-configuration/servers/consul/image.png[alt text]
...
image::02-configuration/servers/consul/image-1.png[alt text]
...
2. The next step is to merge the default values for all active profiles.
...
3. After that, it tries to find more specific settings for the
...
![alt text](/docs/assets/software-engineering/software-architecture/architecture-styles/microservice/02-configuration/servers/consul/image-2.png)
