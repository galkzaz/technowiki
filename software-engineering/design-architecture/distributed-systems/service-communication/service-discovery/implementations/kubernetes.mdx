...
image::07-discovery/servers /Server-side service discovery and load balancing Kubernetes.png[ In Kubernetes, the interprocess communication between Alpha App and Beta App happens through 
...
![A ClusterIP service exposes a set of Pods to the network inside the cluster.](/docs/assets/software-engineering/software-architecture/architecture-styles/microservice/07-discovery/servers /Kubernetes-ClusterIP.png)
...
![The Catalog Service application is exposed to your local machine through port forwarding. Both Catalog Service and PostgreSQL are exposed to the inside of the cluster through the cluster-local hostname](/docs/assets/software-engineering/software-architecture/architecture-styles/microservice/07-discovery/servers /how_the_communication_works_between_your_computer_catalog_service_and_postgresql.png)
