---
id: computer-organization-concepts
title: Introduction
description: computer-organization Introduction
sidebar_position: 1
---

**Computer organization and architecture** define how computers are designed, structured, and implemented. **Architecture** refers to the programmer-visible attributes (instruction sets, data types), while **organization** deals with the operational units (hardware, control signals, memory) that realize these specs. Together, they focus on performance and functional implementation.
...
- **Computer Architecture (What the system does)**: The conceptual design and fundamental operational structure, including Instruction Set Architecture (ISA), data representation (bits), and addressing mechanisms.
- **Computer Organization (How it does it)**: The physical, operational components that implement the architectural specifications, including control signals, interfaces, and memory technology.

Core Components:

- Processor (CPU): Executes instructions, containing the Arithmetic Logic Unit (ALU), registers, and Control Unit.
- Memory Hierarchy: Structures memory for speed and cost efficiency, ranging from fast, expensive cache to slow, cheap storage.
- Input/Output (I/O): Mechanisms for connecting and communicating with peripherals.

Key Differences:

- Architecture: Focuses on design and is often "transparent" to the programmer, focusing on functionality (e.g., for introduction details).
- Organization: Focuses on structural, hardware-level implementation.

**Why Study This Field?**

Understanding this field is essential for optimizing system performance, designing new processors, and understanding how software maps to hardware, such as the, or to learn more.
...
Although a common set of hardware technologies is used in computers ranging from smart home appliances to cell phones to the largest
...
These are general-purpose computers designed for individual use. They usually consist of a desktop or laptopform factor with a CPU, RAM, storage, and input/output devices like a keyboard, mouse, and monitor.
...
Servers are powerful computers that provide services, resources, or data to other computers (clients) over a network. They are optimized for reliability, uptime, and handling multiple requests simultaneously.
...
Supercomputers are extremely powerful machines designed to perform highly complex calculations at immense speeds, often using parallel processing and thousands of processors.
...
These are specialized computers built into other devices to perform dedicated functions. They usually have a microcontroller or microprocessor and are optimized for low power consumption and real-time operation.
...
numbers. For example, the bits `1001010100101110`
...
1. they allow the programmer to think in a more natural language, using English words and algebraic notation, resulting in programs that look much more like text than like tables of cryptic symbols (see Figure above).
2. Moreover, they allow languages to be designed according to their intended use. Hence, Fortran was designed for scientific
   computation, Cobol for business data processing, Lisp for symbol manipulation, and so on. There are also domain-specific languages for even narrower groups ofusers, such as those interested in simulation of fluids, for example.
3. improved programmer productivity. One of the few areas of widespread agreement in software development is that it takes less time to develop programs when they are written in languages that require fewer lines to express an idea. Conciseness is a clear advantage of high-level languages over assembly language.
4. programming languages allow programs to be independent of the computer on which they were developed, since compilers and
   assemblers can translate high-level language programs to the binary instructions of any computer.
...
Figure above shows that these layers of software are organized primarily in a
...
Because accessing memory to get an instruction or data word takes much longer than executing an instruction, all CPUs contain **registers** inside to hold key variables and temporary results. Instruction sets often contains instructions to load a
...
results, most computers have several special registers that are visible to the programmer. 
- One of these is the **program counter**, which contains the memory
...
- Another register is the **PSW (Program Status Word)**. This register contains the condition code bits, which are set by comparison instructions, the CPU
...
The following figure shows the power of the concept; specifically, memory can contain the source code for an editor program, the corresponding compiled machine code, the text that the compiled program is using, and even the compiler that generated the
machine code.
...
**How Fetch–Decode–Execute Works**
1. Fetch
...
Because software compatibility is extremely valuable, the industry tends to converge around a few instruction set architectures. Major modern ISAs include:
...
### smaller is faster
example: The desire to keep all instructions the same size conflicts with the desire to have
...
the similarity of the binary representations of related instructions simplifies hardware design. These similarities are another
...
in this three RISC-V instruction formats R, I, and S. 
- The R-type format has two source register operand and one destination register operand. 
- The I-type format replaces one source register operand with a 12-bit immediate field. 
- The S-type format has two source operands and a 12-bit immediate field, but no destination register operand. The
...
For example, the load register instruction must specify two registers and a constant. If the address were to use one of the 5-bit fields in the format above, the largest constant within the load register instruction would be limited to only $2^5−1$
...
the final hardware design principle: Good design demands good compromises.
