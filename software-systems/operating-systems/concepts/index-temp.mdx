...
When this technique is used, each procedure in the system is free to call any other one, if the latter provides some useful computation that
...
In a monolithic operating system, the entire operating system architecture is built into a single, large kernel that runs in a single address space (kernel space). All core services—including process management, memory management, file systems, and device drivers—are bundled into one large binary. 
...
none—every procedure is visible to every other procedure (as opposed to a structure containing modules or packages, in which much of the information is hidden away inside modules, and only the officially designated entry points can be called from outside the module).
...
it and executes it. The utility procedures do things that are needed by several service procedures, such as fetching data from user programs.
