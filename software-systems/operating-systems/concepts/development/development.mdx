...
## Operating Systems Are Extremely Large and Complex Systems
### No One Understands the Entire OS

This is one of the passage’s most important insights.

Modern operating systems are:

```text id="9zy5ke"
Too large for one person to fully understand
```

Even expert kernel developers usually specialize in:
...
### Specialization in OS Development

Linux kernel maintainers are divided into subsystem experts.
...
| Arch maintainers | CPU architectures |

OS development is highly collaborative.
...
devices such as terminals, pipes, FIFOs, sockets, and regular files. (The kernel translates the application's I/O requests into appropriate filesystem or device-driver operations that perform I/O on the target file or device.)
Thus, a program employing these system calls will work on any type of file.

if we write a program using only these system calls, that
...
$ ./copy /dev/pts/16 /dev/tty Copy input from another termina
```
...
Because details specific to the file system or device are handled within the kernel, we can generally ignore device-specific
factors when writing application programs. When access to specific features of a file system or device is required, a program can use the catchall ioctl() system call, which provides an interface to features that fall outside the universal I/O model.
...
![Linux API](/docs/assets/software-systems/operating-systems/linux/development/Linux_kernel_interfaces.png)
...
![The Linux API is composed out of the system call interface of the Linux kernel](/docs/assets/software-systems/operating-systems/linux/development/Linux_API.png)
...
![Linux API vs POSIX_API](/docs/assets/software-systems/operating-systems/linux/development/Linux_API_vs_POSIX_API.png)
...
![Simplified illustration of the Linux kernel's structure.](/docs/assets/software-systems/operating-systems/linux/development/System_call_Interface.png)
...
| Call | Description 2+^ |
| --- | --- |
| **Process Management** | pid = fork() |
| Create a child process identical to the parent | pid = waitpid(pid, &statloc, options) |
| Wait for a child to terminate | s = execve(name, argv, environp) |
| Replace a process' core image | exit(status) |
| Terminate process execution and return status 2+^ | **File Management** |
| fd = open(file, how, ...) | Open a file for reading, writing, or both |
| s = close(fd) | Close an open file |
| n = read(fd, buffer, nbytes) | Read data from a file into a buffer |
| n = write(fd, buffer, nbytes) | Write data from a buffer into a file |
| position = lseek(fd, offset, whence) | Move the file pointer |
| s = stat(name, &buf) | Get a file's status information 2+^ |
| **Directory and File-System Management** | s = mkdir(name, mode) |
| Create a new directory | s = rmdir(name) |
| Remove an empty directory | s = link(name1, name2) |
| Create a new entry, name2, pointing to name1 | s = unlink(name) |
| Remove a directory entry | s = mount(special, name, flag) |
| Mount a file system | s = umount(special) |
| Unmount a file system 2+^ | **Miscellaneous** |
...
![he 10 steps in making the system call read(fd](/docs/assets/software-systems/operating-systems/linux/development/system-calls-read-call-steps.png)
1. Like nearly all system calls, it is invoked from C programs by cal-
...
C program might look like this:
   ...
   read in count.
2. In preparation for calling the read
...
by convention are used for parameters. 
   ...
   * If there are more than six arguments, the remainder will be
   pushed onto the stack. 
     ...
     buffer
3. Then comes the actual call to the library procedure (step 4). This instruc-
tion is the normal procedure-call instruction used to call all procedures
4. The library procedure, written in assembly language, typically puts the sys-
tem-call number in a place where the operating system expects it, such as the RAX
register (step 5). 
5. Then it executes a trap instruction (such the X86-64 SYSCALL
...
6. The kernel code that starts following the trap examines the system-call number
...
7).
7. at that point, the system-call handler runs (step 8).
8. Once it has completed its
...
9. This procedure then returns to the user pro-
...
handler:
6. Saves register values onto the kernel stack.
7. Checks the validity of the system call number.
8. Invokes the appropriate system call service routine, which is found by
...
9. Restores register values from the kernel stack and places the system call
return value on the stack.
10. Returns to the wrapper function, simultaneously returning the processor
to user mode.
11. If the return value of the system call service routine indicated an error, the
...
![Steps in the execution of a system call](/docs/assets/software-systems/operating-systems/linux/development/Steps-in-the-execution-of-a-system-call.png)
...
1. **Step 1**: User program calls execve()

   ```
   execve('/bin/ls', argv, envp);
   ```
2. **Step 2**: C library prepares the system call
   1. Puts 11 into a register (e.g., EAX)
   2. Puts arguments into other registers
   3. Executes a special instruction (int 0x80 on x86-32)
3. **Step 3**: CPU switches to kernel mode
   1. User mode → kernel mode
   2. Kernel entry code runs
4. **Step 4**: Kernel reads the system call number
   1. syscall_number = 11;
5. **Step 5**: Kernel indexes sys_call_table
   ...
   handler = sys_execve;
   ```
6. **Step 6**: Kernel calls the handler
   ...
   **Step 7: Return to user mode**
   ...
   1. If successful: program image is replaced
   2. If error: return -errno
...
1. **You (user program) say**:
   1. "I want service number 11"
2. **Receptionist (kernel syscall handler)**:
   1. Looks at the service list
   2. Finds service 11 → "Change room"
3. Calls the staff member (kernel function sys_execve)
4. Staff performs the action
...
#### System call charactaristics
##### Atomicity
An operation is atomic if it is executed as one indivisible unit:

* It either happens completely or not at all
* No other process or thread can observe or interfere with intermediate steps

In the context of operating systems: System calls are atomic with respect to other processes and threads.

This means that when a process enters the kernel to execute a system call, the kernel ensures that the internal steps of that call are not interleaved with steps from another process performing the same shared-kernel operation.

Atomicity does not mean:

* The system call cannot be preempted at all
* The CPU cannot switch to another process while it runs

Instead, it means:

* The kernel protects shared data structures
* Partial results are never visible
* Kernel invariants are preserved

This is typically achieved using:

* Interrupt disabling (for very short sections)
...
* The final result depends on the timing or interleaving of execution

The "race" is about who gets there first.

Consider this POSIX operation:
```
pid = fork();
```
From user space, fork() looks like a single step, but internally the kernel:

* Allocates a process descriptor
* Copies page tables
* Assigns a PID
* Updates scheduler structures

All of this is done atomically:

* No other process sees a "half-created" process
* The child either fully exists or does not exist at all

Atomicity Prevents Race Conditions, Atomicity ensures:

* Consistency of kernel data structures
* Correct behavior under concurrency
* Predictable outcomes, regardless of scheduling order

Without atomicity:

* File systems would corrupt metadata
* Process tables would become inconsistent
* Reference counters would underflow or overflow
* Security guarantees would break
...
##### Atomic vs Non-Atomic POSIX Operations
Atomic POSIX operation: Guaranteed by POSIX to complete as a single, indivisible action with respect to other processes or threads.

Non-atomic operation: Can be interrupted or interleaved, exposing intermediate state and enabling race conditions.

POSIX does not say "system calls are always atomic"; it says specific operations are atomic under specific conditions.

This distinction is crucial in:

* File systems
* Concurrent programming
* Security (TOCTTOU vulnerabilities)

###### File Operations

**Atomic: open() with O_CREAT | O_EXCL**

int fd = open('data.txt', O_CREAT | O_EXCL | O_WRONLY, 0644);

File creation and existence check occur as one operation

Prevents check-then-create race conditions

Either the file is created or the call fails with EEXIST
...
```
Another process may create the file between access() and open().
...
Commonly used for safe updates and commits

**Non-atomic Alternative**

unlink('final.txt');
link('temp.txt', 'final.txt');

Between calls, final.txt may not exist.

###### Read and Write Operations

**Atomic: write() to Pipe or FIFO**

write(fd, buffer, size);

Atomic if:
** File descriptor refers to a pipe or FIFO
** size &lt;= PIPE_BUF

Writes are not interleaved with other writers

**Non-atomic write()**

write(fd, buf1, 6000);

If 6000 > PIPE_BUF, data may interleave with other writers.
...
Multiple processes can overwrite each other's updates.

###### File Offset Manipulation

**Non-atomic: lseek() + write()**

lseek(fd, 0, SEEK_END);
write(fd, data, len);

Another process may write between the two calls.

**Atomic: write() with O_APPEND**

fd = open('log.txt', O_WRONLY | O_APPEND);
write(fd, data, len);

File offset adjustment and write occur atomically

Widely used for logging and auditing
...
Child process is either fully created or not created at all

No partially initialized process is visible

**Non-atomic Pattern After fork()**

pid = fork();
shared_counter++;

Incrementing shared state is not atomic.
...
Signal handler replacement is atomic.

**Non-atomic Signal Mask Handling**

sigprocmask(SIG_BLOCK, &set, NULL);
critical_section();
sigprocmask(SIG_UNBLOCK, &set, NULL);

Incorrect handling may lead to lost or unexpected signals.
...
These are defined as atomic synchronization points.

**Non-atomic Shared Variable Access**

counter++;

Not atomic at the POSIX level, even on single-core systems.

###### Summary Table

|     |     |     |
| --- | --- | --- |
| Operation | Atomic | Conditions |
| open(O_CREAT | O_EXCL) | Yes |
| Always | rename() | Yes |
| Same filesystem | write() to pipe | Yes |
| size &lt;= PIPE_BUF | write() with O_APPEND | Yes |
| Always | fork() | Yes |
| Always | read() + write() | No |
| Race-prone | lseek() + write() | No |
| Race-prone | access() + open() | No |

##### Linux-specific Guarantees vs POSIX Guarantees

POSIX defines a portable minimum contract

Linux often provides stronger, non-portable guarantees

Correct systems programming means:
** Rely on POSIX guarantees for correctness
** Use Linux guarantees only as explicit optimizations

If POSIX guarantees it, it is safe everywhere.
If Linux guarantees it, it is safe only on Linux.
...
Designed for portability across Unix-like systems

Specifies observable behavior, not implementation

Avoids overspecifying atomicity or ordering
...
Often stronger due to kernel internals

May change across kernel versions (rare, but allowed)
...
Partial writes and interleaving allowed otherwise

**Linux Guarantee**

For regular files:
** A single write() is not interleaved with other writes
** The write may still be split internally

This behavior is Linux-specific and not POSIX-mandated

Do not rely on non-interleaving writes for portable code.
...
Either old name or new name exists

No partially visible state

**Linux Guarantee**

Same atomicity as POSIX

Journaling filesystems (ext4, xfs) provide crash consistency

Commonly used as a transactional primitive

POSIX defines what must happen; Linux strengthens how safely it happens.

###### File Offsets and O_APPEND

**POSIX Guarantee**

File offset repositioning and write occur atomically

Prevents race conditions between writers

**Linux Guarantee**

Implemented using inode-level locking
...
Linux strictly conforms to POSIX here.
...
Prevents TOCTTOU race conditions

**Linux Guarantee**

Same observable behavior

Enforced via VFS and inode locks

Linux matches POSIX exactly.

###### pread() and pwrite()

**POSIX Guarantee**

Atomic with respect to file offset

Do not modify the shared file position

**Linux Guarantee**

Implemented by passing explicit offsets to the filesystem

No shared-offset race possible

Linux provides stronger practical isolation, but semantics are POSIX-defined.

###### Signals

**POSIX Guarantee**

Signal handler installation is atomic

Handler executes with the signal blocked

Ordering between signals is unspecified

**Linux Guarantee**

Signals delivered per-thread

Ordering is implementation-defined

Fully POSIX-compliant, but not stronger

Never rely on Linux signal ordering.
...
pthread_mutex_lock() is atomic

Provides mutual exclusion and memory ordering

**Linux Guarantee**

Implemented using futexes

Avoids kernel entry when uncontended

Stronger performance, same semantics

Linux optimizes behavior without extending guarantees.
...
Child process created atomically
...
Linux improves performance, not correctness guarantees.
...
No atomicity guarantee for plain memory writes

Explicit synchronization required

**Linux / Architecture Guarantee**

On x86:
** Naturally aligned word writes are atomic

On ARM:
** Atomicity depends on alignment and CPU features

This is hardware- and Linux-specific and must not be relied upon for portable code.

###### Summary Table

| Operation | POSIX Guarantee | Linux Guarantee |
| --- | --- | --- |
| write() to pipe | Atomic ≤ PIPE_BUF | Same |
| write() to regular file | No atomicity | Non-interleaved (Linux-only) |
| rename() | Atomic | Atomic + journaling safety |
| O_APPEND write | Atomic | Atomic |
| pread() / pwrite() | Offset-safe | Offset-safe |
| fork() | Atomic creation | Copy-on-write optimized |
| Mutex lock | Atomic | Futex-optimized |
| Plain memory write | No guarantee | Architecture-dependent |

##### Bugs Caused by Relying on Linux-only Guarantees

###### Assuming write() to Regular Files Is Atomic

**Buggy Linux-only Assumption**

A single write() to a regular file will never interleave with another write().

This behavior is observed on Linux but not guaranteed by POSIX.
...
On non-Linux POSIX systems or network filesystems, output may interleave:

PID=123
PID=PID=456
789

Writes from different processes mix together.
...
fd = open('config.cfg', O_RDONLY);
}
```
**Why It Breaks**

Another process can replace or modify the file between access() and open().
This pattern has caused real privilege escalation vulnerabilities.
...
Or, for creation:
```
open('file', O_CREAT | O_EXCL, 0644);
```
###### Relying on x86 Atomic Memory Writes

**Buggy Assumption**

Incrementing a shared variable is atomic because it works on Linux/x86.
...
Appears correct on x86 Linux

Fails on ARM, RISC-V, PowerPC

Lost updates and corrupted state

POSIX provides no atomicity guarantee for plain memory operations.
...
atomic_int counter;
```
###### Assuming Signal Delivery Order

**Buggy Assumption**

Signals are delivered in the order they are sent.
...
POSIX: signal ordering is unspecified

Linux: ordering varies with load and kernel version
...
Realtime signals (SIGRTMIN + n)

signalfd

Pipes or eventfd for ordered events

###### Assuming fork() Duplicates All Threads

**Buggy Assumption**

All threads survive across fork().
...
Mutexes may remain locked forever

Leads to deadlocks

**POSIX Rule**

After fork() in a multithreaded program:

Only async-signal-safe functions are allowed

Must immediately call exec()
...
```
###### Assuming Journaling Equals Crash Safety

**Buggy Assumption**
```
rename() is always crash-safe.
```
**Reality**

POSIX guarantees atomic name replacement

POSIX does not guarantee durability

Power loss may cause data loss
...
fsync(dir_fd);
```
Without fsync(), data may not survive crashes.

###### Treating /proc as a POSIX Filesystem

**Buggy Assumption**

/proc files behave like normal files.

**What Goes Wrong**

lseek(), stat(), read() behave inconsistently

Code breaks on BSD, macOS, Solaris

**Correct Approach**

Treat /proc as Linux-specific

Isolate platform-dependent code

Never assume POSIX semantics

###### Summary

| Cause | Result |
| --- | --- |
| Linux behavior assumed portable | Non-portable bugs |
| x86 assumptions | ARM and RISC failures |
| Fast local testing | Hidden race conditions |
| Ignoring POSIX rules | Security vulnerabilities |

###### Core Rule

Linux guarantees are performance hints.
POSIX guarantees are correctness contracts.

Rely only on POSIX guarantees for portable, correct systems code.
==== Examples and tools
...
2. or by calling a library function. 

   To handle this possibility, a
   program can 

   1. call the gnu_get_libc_version() function to determine the version of
   ...
   2. We can also obtain version information by using the confstr() function to
...
![OpenGL is indeed an abstraction API to make use of diverse GPUs of multiple vendors without the need to program for each one specifically.](/docs/assets/software-systems/operating-systems/linux/development/Linux_kernel_and_OpenGL_video_game.png)
...
1. One way that we can do
this is by defining the macro in the program source code before including any
header files:

       #define _BSD_SOURCE 1
2. **Alternatively, we can use the -D option to the C compiler**:
