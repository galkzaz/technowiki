...
2. The second way is to make an entry in an operating system
...
3. The third way is for the operating system to be able to
...
2. The second method is for the driver to start the device and ask it to give an
...
tion.
3. The third method for doing I/O makes use of special hardware: a DMA
...
| -a | Append output to the end of the file instead of overwriting |
...
2. Open the output file using appropriate flags
3. Repeatedly read data from standard input
4. Write the data to:
...
| --- | --- | --- |
| read() | &lt;unistd.h> | Read bytes from standard input |
| write() | &lt;unistd.h> | Write bytes to standard output and file |
| open() | &lt;fcntl.h> | Open or create the output file |
...
| write | Write non-zero data to the destination file |
| lseek | Skip over zero-filled regions to create holes |
...
3. Read a block of data from the source file
4. Check whether the block contains only zero bytes
5. If the block is all zeros, advance the destination offset using `lseek`
6. Otherwise, write the block to the destination file
