...
devices. (The kernel translates the application’s I/O requests into appropriate filesystem or device-driver operations that perform I/O on the target file or device.)
...
+
* **Concept**\
1️⃣ Every process has file descriptors (FDs)
  ...
  2️⃣ /proc is a live view into the kernel 🧠

  /proc is not a real filesystem on disk.
  It is a window into the kernel’s internal data.
  ...
  3️⃣ /proc/PID/fdinfo → detailed FD information
  ...
  4️⃣ What’s inside an fdinfo file?
  ...
  5️⃣ pos → current file position (offset)
  What it means

  pos tells you:

  “Where in the file will the next read or write happen?”
  ...
  6️⃣ flags → how the file was opened
  What it is
  ...
  7️⃣ Why flags are shown as numbers
  ...
  8️⃣ Why the book says “look at C library header files”
  ...
  Humans understand symbolic names (O_RDONLY, O_APPEND, …)
...
I’ll map everything you saw in /proc/PID/fdinfo to struct file, using simple language and concrete kernel references.
...
+
Mapping /proc/PID/fdinfo → struct file
1️⃣ fd number → pointer to struct file
+
When you see:
+
/proc/1234/fdinfo/3
...
+
fd      ─┐
         ├──► struct file (shared f_pos)
fd2     ─┘
+
That’s why reading via one FD moves the offset for the other.
+
3️⃣ flags: → file->f_flags
/proc output
flags: 0100002
+
Kernel field
...
+
4️⃣ Why /proc shows octal numbers
+
...
Mount namespaces
+
6️⃣ inode linkage (not shown directly)
+
Even though /proc/PID/fdinfo doesn’t show it, every struct file links to:
+
struct inode *f_inode;
+
...
Result:
+
fd1 ─► struct file (flags=RDONLY, pos=0)
fd2 ─► struct file (flags=WRONLY, pos=0)
             │
             └── same inode
+
8️⃣ How /proc/PID/fdinfo is generated
+
...
file->f_path
+
So /proc is not magic — it is formatted kernel memory.
+
Mental Model (Remember This)
FD number (int)
   ↓
struct file  ←── this is what fdinfo shows
   ├── f_pos   → pos:
   ├── f_flags → flags:
   ├── f_path  → mnt_id
   └── f_inode → actual file
...
Attempts to modify other flags are ignored. (Some other UNIX imple-
...
image::{figures}/file-system-dev-Failing-to-exclusively-create-a-file.png[Failing to exclusively create a file]
