...
id: process
title: Process
description: Operating Systems Process
---

The most central concept in any operating system is the
process: an abstraction of a running program. Everything else hinges on this concept, and the operating system designer (and student) should have a thorough understanding of what a process is as early as possible.
...
could be dozens or hundreds of processes running. Without the process abstraction, modern computing could not exist.

All modern computers often do several things at the same time. In any multiprogramming system, each CPU switches from process to process quickly, running each for tens or maybe hundreds of milliseconds. While strictly
speaking, **at any one instant each CPU is running only one process**, in the course of
1 second it may work on several of them, giving the illusion of parallelism. Sometimes people speak of pseudoparallelism in this context, to contrast it with the
...
years have evolved a conceptual model (sequential processes) that makes parallelism easier to deal with.
...
CPU switches back and forth from process to process, but to understand the system, it is much easier to think about a collection of processes running in (pseudo)
...
our baker reading the recipe, fetching the ingredients, and baking the cake. Now imagine that the computer scientist’s son comes running in screaming his
...
reserved area of disk space used to supplement the computer’s RAM). 
* Similarly, a
process doesn’t know where on the disk drive the files it accesses are being held; it
simply refers to the files by name. 
* A process operates in isolation; it can’t directly
communicate with another process. 
* A process can’t itself create a new process or
even end its own existence. 
* a process can’t communicate directly with the
...
When we say things such as “a process can create another pro-
...
2. Transitions 2 and 3 are caused by the process scheduler, a part of the operating
...
3. Transition 3 occurs when all the other
...
4. Transition 4 occurs when the external event for which a process was waiting
...
4. **Initiation of a batch job**
   ...
   a chain of stores—calculating what to order, analyzing per-store product popular-
...
5️⃣ The classic two-step process: fork() → execve()
...
6️⃣ Why not just “create a process and run a program” in one step?
...
7️⃣ File descriptors (FDs) in simple terms
...
8️⃣ Why fork() first helps with redirection
...
💡 sort doesn’t know about files or redirection at all
...
ified. Alternatively, the child may share all of the parent’s memory, but in that case
...
share some of its creator’s other resources, such as open files. 
...
* **Concept**
  ...
  1. What is an “address space”?
     ...
     It cannot see or touch another process’s memory

     The OS + MMU enforce this separation

     👉 Think of it as each process living in its own private apartment.
  ...
  3. Traditional UNIX: “copy of the parent’s memory”

     In classic UNIX:

     The child’s memory is initially a copy of the parent’s memory
  ...
  4. Sharing program text (code) is safe
     ...
     👉 This saves memory and is safe because code does not change.
  ...
  6. “No writable memory is shared” (IMPORTANT)
     ...
     ➡️ Writable memory is never shared by default
  ...
  Address space	Process’s private memory
...
Processes frequently need to synchronize and communicate with other proc-
esses. For example, in a shell pipeline, the output of the first process must be pas-
sed to the second process, and so on down the line. Thus, there is a need for com-
munication between processes, preferably in a well-structured way not using inter-
rupts.
